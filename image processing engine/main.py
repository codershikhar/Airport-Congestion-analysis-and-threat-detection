...
class ServiceTracker(threading.Thread):
    flag1 = 0
    oldFrame = None
    p0 = np.array([[60,100]],dtype=np.float32)
    p1 = None
    ...
    def trackMyGuy(self):
        while True:
            ...
            if self.flag1 == 1:
                ...
                if(x<10):
                    #print("srv time : " +feedInfo[self.camindex][1]+" "+str(int(3*350*(time.time()-self.start_time))))
...
class BagDetect(threading.Thread):
    frame = None
    check = []

    def __init__(self, frame, camindex):
        threading.Thread.__init__(self)
        self.frame = frame
    ...
    def doStuff(self):
        self.frame = globFrames[self.camindex]
        self.frame = cv2.cvtColor(self.frame, cv2.COLOR_BGR2GRAY)
        baggage = bag.detectMultiScale(self.frame, 2, 3)
        for (x, y, w, h) in baggage:
            cv2.rectangle(self.frame, (x, y), (x + w, y + h), (0, 0, 255), 2)
            temp_cood = [x, y, time.time()]
            self.check.append(temp_cood)
            for a in self.check:
                #print "Baggage Spotted"
                if (temp_cood[0] == a[0] and temp_cood[1] == a[1]):
                    if (temp_cood[2] > a[2] + 0.02):
                        threatFrame = self.frame[y:y+h, x:x+w]
                        cv2.imshow("Threats", threatFrame)
                        cv2.waitKey(1)
                        os.system("vlc alert.mp3")
                        cv2.rectangle(globFrames[self.camindex],(x,y),(x+w, y+h), (0,0,255),2)
...
class Feed (threading.Thread):
    ...
    def people_count(self):
        ...
        print("number of human features detected @ " + self.camname + " " + str(self.numberOfPeople))
    ...
    def run(self):
        ...
        trackthd.start()
        bagtd = BagDetect(self.frame, self.index)
        bagtd.start()
        cnt=0
        while ret:
            #print ("speed fps : " + str((self.cap.get(cv2.CAP_PROP_POS_FRAMES) / (time.time() - self.strt_time)) / self.cap.get(cv2.CAP_PROP_FPS)))
